...
public class SharedKeyCreationAction extends AbstractAction implements NotifiableAction {
	...
	public void run(ActionContext actionContext) {
		...
		String publicKey;
		try {
			publicKey = ((DSAPublicKey) this.tenantManager.getPublicKey(tenant)).getY().toString(10);
		} catch (UnivoteException ex) {
			logger.log(Level.INFO, "Public key for tenant not available.");
			this.informationService.informTenant(skcac.getActionContextKey(), "Public key for tenant not available.");
			this.actionManager.runFinished(actionContext, ResultStatus.FAILURE);
			return;
		}
		...
		try {
			...
			try {
				...
				enhancedEncryptionKeyShare = createEncryptionKeyShare(publicKey, cryptoSetting);
