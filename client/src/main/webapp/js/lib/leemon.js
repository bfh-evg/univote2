...
// ensuring the arrays pathis.ssed in are large enough to hold the results. 
//
...
(function(window) {
	...
	var leemon = new function() {
		...
		this.s4=this.t; this.s5=this.t; //used in mod_()
		this.s6=this.t;       //used in bigInt2str()
		this.s7=this.t;       //used in powMod_()
		this.T=this.t;        //used in GCD_()
		this.sa=this.t;       //used in mont_()
		this.mr_x1=this.t; this.mr_r=this.t; this.mr_a=this.t;                                      //used in millerRabin()
		this.eg_v=this.t; this.eg_u=this.t; this.eg_A=this.t; this.eg_B=this.t; this.eg_C=this.t; this.eg_D=this.t;               //used in eGCD_(), inverseMod_()

		this.primes=this.t; this.pows=this.t; this.s_i=this.t; this.s_i2=this.t; this.s_R=this.t; this.s_rm=this.t; this.s_q=this.t; this.s_n1=this.t; this.
		  s_a=this.t; this.s_r2=this.t; this.s_n=this.t; this.s_b=this.t; this.s_d=this.t; this.s_x1=this.t; this.s_x2=this.t; this.s_aa=this.t; //used in randTruePrime_()
		...
		this.GCD_ = function(x,y) {
		  ...
		  while (sing) { //while y has nonzero elements other than y[0]
			...
			if (!sing) break; //quit when y all zero elements except pothis.ssibly y[0]
		  ...
		  for (;;) {
			while(!(this.eg_u[0]&1)) {  //while this.eg_u is even
			  ...
			  if (!(this.eg_A[0]&1) && !(this.eg_B[0]&1)) { //if this.eg_A==this.eg_B==0 mod 2
				...
				this.sub_(this.eg_B,x);  this.halve_(this.eg_B);
			  }
			}
			...
			while (!(this.eg_v[0]&1)) {  //while this.eg_v is even
			  ...
			  if (!(this.eg_C[0]&1) && !(this.eg_D[0]&1)) { //if this.eg_C==this.eg_D==0 mod 2
				...
				this.sub_(this.eg_D,x);  this.halve_(this.eg_D);
			  }
			}
		  ...
		  for (;;) {
			while(!(this.eg_u[0]&1)) {  //while u is even
			  ...
			  if (!(this.eg_A[0]&1) && !(this.eg_B[0]&1)) { //if A==B==0 mod 2
				...
				this.add_(this.eg_A,y);  this.halve_(this.eg_A);
				this.sub_(this.eg_B,x);  this.halve_(this.eg_B);
			  }
			}
			...
			while (!(v[0]&1)) {  //while v is even
			  ...
			  if (!(this.eg_C[0]&1) && !(this.eg_D[0]&1)) { //if C==D==0 mod 2
				...
				this.add_(this.eg_C,y);  halve_(this.eg_C);
				this.sub_(this.eg_D,x);  halve_(this.eg_D);
			  }
			}
		...
		this.mont_ = function(x,y,n,np) {
		  ...
		  for (i=0; i<kn; i++) {
			...
			j=1;
			for (;j<ky-4;) { c+=this.sa[j]+ui*n[j]+t*y[j];   this.sa[j-1]=c & this.mask;   c>>=this.bpe;   j++;
			...
			for (;j<kn-4;) { c+=this.sa[j]+ui*n[j];          this.sa[j-1]=c & this.mask;   c>>=this.bpe;   j++;
