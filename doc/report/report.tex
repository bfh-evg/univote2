...
\newcommand{\BB}{\ensuremath{\mathit{BB}}\xspace}
...
\chapter{Cryptographic System Specification}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
	\item Identifier: \BB
	\item Public certificate: $Z_{\BB}$, signed by \CA at time $t$
	\item Public verification key: $\VK{\BB}$ 
	\item Private signature key: $\SK{\BB}$
\end{itemize}
...
\BB notifies registered listeners about published messages. Anybody can register itself as a listener.
...
\begin{enumerate}
	...
	\item Select $Z_{\EA}$ from the \BB. Check that $\mathit{Verify}_{\VK{\CA}}(Z_{\EA})=\mathit{accept}$.
	\item Generate signature $S_{\EA}=\mathit{Sign}_{\SK{\EC}}(\eid||Z_{\EA})$. 
	\item Publish $(\EC,\eid,Z_{\EA},S_{\EA})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item For each $\Tallier{j}\in T$, select $Z_{j}$ from \BB and check that $\mathit{Verify}_{\VK{\CA}}(Z_{j})=\mathit{accept}$. Let $\mathcal{Z}_{T}=\{Z_j:1\leq j\leq r\}$.
	\item Define mixers $M=\{\Mixer{1}\ldots,\Mixer{m}\}$.
	\item For each $\Mixer{k}\in M$, select $Z_{k}$ from \BB and check that $\mathit{Verify}_{\VK{\CA}}(Z_{k})=\mathit{accept}$. Let $\mathcal{Z}_{M}=\{Z_k:1\leq k\leq m\}$.
	\item Generate signature $S_{\descr}=\mathit{Sign}_{\SK{\EA}}(\eid||\descr||\ell||\mathcal{Z}_{T}||\mathcal{Z}_{M})$.
	\item Publish $(\EA,\eid,\descr,\ell,\mathcal{Z}_{T},\mathcal{Z}_{M},S_{\descr})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\EC,\eid,P,Q,G,S_{\mathit{PQG}})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\Tallier{j},\eid,y_j,\pi_{x_j},S_{y_j})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,y,S_y)$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\Mixer{k},\eid,g_k,\pi_{\alpha_k},S_{g_k})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\hat{g},S_{\hat{g}})$ on \BB. 
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\EA,\eid,C,R,S_C)$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\EC,\eid,\EA,\descr,P,Q,G,y,\hat{g},C,R,S_{data})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\EA,\eid,H_V,S_{V})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\mathcal{Z}_V,S_V)$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\Mixer{k},\eid,\mathit{VK}_k,\pi_{\psi_k},S_{\mathit{VK}_k})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\mathit{VK}',S_{\mathit{VK}'})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	\item Retrieve $(\EC,\eid,\EA,\descr,P,Q,G,y,\hat{g},C,R,S_{data})$ from \BB.\footnote{$\EA$ and $\descr$ are not explicitly required in the following steps. But it is important for $\Voter{i}$ to learn the identity of the election administration and the content of the election.}
	...
	\item Publish ballot $B_i=(\vkprime{j},\eid,E_i,\pi_{r_i},S_i)$ on \BB.
\end{enumerate}
Upon receipt of $B_i$, \BB performs the following tests:
\begin{enumerate}[resume]
	...
	\begin{enumerate}
		\item Check that no ballot on \BB contains $\vkprime{j}$.
		\item If $\vkprime{j}\in \bar{VK}'\cup\hat{VK}'$, check that no other ballot on \BB contains a different key of $\Voter{i}$ from $\bar{VK}'\cup\hat{VK}'$.
	\end{enumerate}
	...
	\end{enumerate}
\end{enumerate}
\BB publishes $B_i$, if all tests succeed.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item If $\vkprime{j}\in \bar{VK}'\cup\hat{VK}'$, check that no other (more recent) ballot on \BB contains a different key of $\Voter{i}$ from $\bar{VK}'\cup\hat{VK}'$.
		\item Check that $\mathit{Verify}_{\vkprime{j}}(\eid||E_i||\pi_{r_i},S_i)=\mathit{accept}$ using $\hat{g}$.
	...
	\item Publish $(\EC,\eid,\mathcal{B},S_{\mathcal{E}})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\Mixer{k},\eid,\mathcal{E}_k,\pi_{\tau_k},S_{\mathcal{E}_k})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\mathcal{E}',S_{\mathcal{E}'})$ on \BB.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Publish $(\Tallier{j},\eid,\bar{a}_j,\pi_{x_j}',S_{\bar{a}_j})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\mathcal{V},S_{\mathcal{V}})$ on \BB.
\end{enumerate}
...
Let $\Voter{i}$ be an eligible voter in a current election. This can be tested by checking if $H(\Voter{i})\in H_V$, where $V$ is denotes the set of eligible voters and $H_V$ the corresponding set of hash values as published on \BB (see \subsecref{preparation}). A late registration invokes the procedure described below.
...
\begin{enumerate}
	...
	\item Publish $(\EC,\eid,\bar{Z}_i,S_{\bar{Z}_i})$ on \BB. Let $\bar{\mathcal{Z}}_V$ denote the current set of certificates added during the election period.
\end{enumerate}
...
\	\item Generate $\pi_{\vkbar{i,k}}=\mathit{NIZKP}\{(\alpha_k):g_k=g_{k-1}^{\alpha_k}\wedge\vkbar{i,k}=\vkbar{i,k-1}^{\alpha_k}\}$ (see \subsecref{proof_reg_new} for details).
	...
	\item Publish $(\Mixer{k},\eid,\vkbar{i,k},\pi_{\vkbar{i,k}},S_{\vkbar{i,k}})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\Voter{i},\vkbarprime{i},S_{\vkbarprime{i}})$ on \BB. Let $\bar{\mathit{VK}}'$ denote the set of all public keys $\vkbarprime{i}$ added during the election period.
\end{enumerate}
...
\	\item Generate $\pi_{\vkhat{i,k}}=\mathit{NIZKP}\{(\alpha_k):g_k=g_{k-1}^{\alpha_k}\wedge\vkhat{i,k}=\vkhat{i,k-1}^{\alpha_k}\}$ (see \subsecref{proof_reg_new} for details).
	...
	\item Publish $(\Mixer{k},\eid,\vkhat{i,k},\pi_{\vkhat{i,k}},S_{\vkhat{i,k}})$ on \BB.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Publish $(\EC,\eid,\Voter{i},\vkhatprime{i},S_{\vkhatprime{i}})$ on \BB. Let $\hat{\mathit{VK}}'$ denote the set of all public keys $\vkhatprime{i}$, which have been replaced by a new one during the election period.
\end{enumerate}
