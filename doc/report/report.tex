...
\begin{enumerate}
	...
	\item $UC$ checks with $a_{resp}$ if $R$ is authorized to generate a certificate for $u$ \todo{is it needed? If yes, how is that done?}
	\item $UC$ checks if $\pi$ is correct
	\item $UC$ checks if $s$ used by $R$ to generate $x$ and $y$ is the same as $s'$ corresponding to $u$
	\item $UC$ get the properties $\mathcal{P}$ linked to $u$ from $UB$ \todo{get()}
