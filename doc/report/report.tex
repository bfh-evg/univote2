...
\begin{document}
	...
	\begin{versionhistory}
		...
		\vhEntry{0.6.1}{16.06.2015}{Severin Hauser}{Revision of UniVote.}
		\vhEntry{0.6.2}{16.06.2015}{Reto E. Koenig}{Revision of UniVote.}
		\vhEntry{0.6.3}{26.06.2015}{Rolf Haenni}{Table in Section 7 adjusted.}
		\vhEntry{0.6.4}{20.07.2015}{Rolf Haenni}{Updated tables in Section 7; Updated late registration procedure.}
...
\begin{enumerate}
	...
	\end{itemize}
	according to $\securityLevel$, where $\Sigma_\mathsf{ENCRYPT}$, $\Sigma_\mathsf{SIGN}$, $\Sigma_\mathsf{HASH}$ are corresponding sets of predefined cryptographic settings for ElGamal encryptions, Schnorr signatures, and hash functions, respectively (see \Cref{settings}).
...
Let $\vk{}=\vk{0}$ be a single key to be ``mixed'' in essentially the same way as described in \Cref{preparation}. A simplified sub-routine for this is called twice in the above general procedure for different input keys. For this, the following steps are repeated for every $\Mixer{k}\in M$ (in ascending order for $1\leq k\leq m$):
\begin{enumerate}[]
	\item \EC performs the following steps:
	\begin{enumerate}
		\item Post $(\mathsf{singleKeyMixingRequest}:\Mixer{k}, \vk{k-1})$ to \UBV.
		\item Select $\vk{k}$ from $Z_{k}\in \mathcal{Z}_{M}$.
		\item Post $(\mathsf{accessRight}:\vk{k},\mathsf{singleKeyMixingRequest},x=x+1)$ to \UBV.
	\end{enumerate}
	\item $\Mixer{k}$ performs the following steps:
	\begin{enumerate}
		\item Get $(\mathsf{singleKeyMixingRequest}: \Mixer{k}, \vk{k-1})$ from \UBV.
		\item Compute $\vk{k} = \vk{k-1}^{\alpha_k}$
		\item Generate $\pi_{k}=\mathit{NIZKP}\{(\alpha_k):g_k=g_{k-1}^{\alpha_k}\wedge \vk{k} = \vk{k-1}^{\alpha_k}$.
		\item Post $(\mathsf{singleKeyMixingResult}:\vk{k-1},\vk{k}, \pi_{k})$ to \UBV.
	\end{enumerate}
	\item \EC performs the following steps:
	\begin{enumerate}
		\item Get $(\mathsf{singleKeyMixingResult}:\vk{k-1},\vk{k}, \pi_{k})$ from \UBV.
		\item Verify $\pi_{k}$.
	\end{enumerate}
\end{enumerate}
Let $\vkhat{}=\vk{m}$ denote the result of this sub-routine.
...
\begin{center}
	\begin{table}[ht]
		\begin{tabular}{c|l|c|l|c}
			...
			\EC & $\mathsf{cryptoSetting}$ & 1 & $\mathit{encryptSetting},\mathit{signSetting},\mathit{hashSetting}$ & \Tallier{j}, \Mixer{k} \\
			...
			\EC & $\mathsf{votingData}$ & 1 & $\electionTitle, \electionPeriod, \mathcal{Q},\mathcal{R}, \electionIssues, \mathit{encryptSetting},$ & \Voter{i} \\
			& & & $\mathit{signSetting},\mathit{hashSetting},y, \hat{g}$ \\
...
\begin{table}[ht]
	\begin{center}
		\begin{tabular}{l|l|l}
			...
			$\electionPeriod=(t_{1},t_{2})$ & time period & \\
			...
			$\mathcal{Q}=\{Q_1,\ldots,Q_s\}$ & option list & \\
			$\mathcal{R}$ & rule list &  \\
			...
			$\mathcal{W}=\{w_1,\ldots,w_{N}\}$ & list of decrypted votes & \\
			$\mathcal{V}=\{v_1,\ldots,v_{N'}\}$ & list of decoded votes & \\
...
To allow a variety of different election types, we consider two finite sets, a set $\mathcal{Q}=\{Q_1,\ldots,Q_s\}$ of possible \emph{election options} and a set $\mathcal{R}$ of \emph{election rules}. For each election option $Q_j\in \mathcal{Q}$ in a given election, the election system outputs the number $r_j$ of votes that $Q_j$ has received from the voters. Each election rule in $\mathcal{R}$ defines some constraints on how voters can distribute their votes among the election options. We use $v_{i,j}$ to denote this number for a particular ballot $B_i$ and an election option $Q_j$. The tuple $v_i=(v_{i,1},\ldots,v_{i,s})\in\validVotes(\mathcal{Q},\mathcal{R})$ represents a valid vote, where $\validVotes(\mathcal{Q},\mathcal{R})$ denotes the set of all valid votes for given sets $\mathcal{Q}$ and $\mathcal{R}$.
...
\begin{itemize}
	...
	\begin{align*}
		\mathcal{Q}=\{yes,no\},~ 
		\mathcal{R}=\left\{\begin{aligned}\Sigma:\{yes,no\}\rightarrow[1,1] \\ \forall:\{yes,no\}\rightarrow[0,1]\end{aligned}\right\}
	\end{align*}
	
	\item Referendum with Null Votes: max-$1$-out-of-$2$
	\begin{align*}
		\mathcal{Q}=\{yes,no\},~
		\mathcal{R}=\left\{\begin{aligned}\Sigma:\{yes,no\}\rightarrow[0,1] \\ \forall:\{yes,no\}\rightarrow[0,1]\end{aligned}\right\}
	\end{align*}
	or
	\begin{align*}
		\mathcal{Q}=\{yes,no,null\},~ 
		\mathcal{R}=\left\{\begin{aligned}\Sigma:\{yes,no, null\}\rightarrow[1,1] \\ \forall:\{yes,no, null\}\rightarrow[0,1]\end{aligned}\right\}
	\end{align*}	
	\item Referendum List: $s$ times $1$-out-of-$2$
	\begin{align*}
		\mathcal{Q}=\{yes_1,no_1,\ldots,yes_s,no_s\},~ 
		\mathcal{R}=\left\{\begin{aligned}&\Sigma:\{yes_1,no_1\}\rightarrow[1,1] \\
		&~~~\vdots~~~~~~~~~~~\vdots~~~~~~~~~~~\vdots\\
		&\Sigma:\{yes_s,no_s\}\rightarrow[1,1] \\
		&\forall:\{yes_1,no_1,\ldots,yes_s,no_s\}\rightarrow[0,1]\end{aligned}\right\}
	\end{align*}
	...
	or
	\begin{align*}
		\mathcal{Q}=\{Q_1,\ldots,Q_n,null\},~ 
		\mathcal{R}=\left\{\begin{aligned}\Sigma:\{Q_1,\ldots,Q_n, null\}\rightarrow[1,1] \\ \forall:\{Q_1,\ldots,Q_n, null\}\rightarrow[0,1]\end{aligned}\right\}
	\end{align*}
...
%	\end{align*}
	...
	\begin{center}
		\begin{tabularx}{\textwidth}{|X|c|c|c|c|c|}\hline
			\centering\multirow{2}{*}{Level} & \multirow{2}{*}{Strength} & \multicolumn{3}{c|}{Residue Class Group} & \multirow{2}{*}{Hash Value} \\\cline{3-5}
			& &  modulo & max.\ order & min.\ order & \\\hline
			\centering 0 (for testing only) & 2   & 6    & 5    & 4  & 4 \\
...
With $\rho_j$ we denote the maximum number of votes that the election rules in $R$ allow for $c_j$.
 
To encode $v_i\in \mathsf{Votes}$ as an integer, consider a bit string of length $B=\sum_{j=1}^n b_j$, where $b_j=\lfloor\log_2\rho_j\rfloor+1$ denotes the number of bits reserved for each value $v_{i,j}$. Furthermore, let $B_j=\sum_{k=1}^{j-1}b_k$ be the number of bits in the bit string \emph{prior} to $v_{i,j}$, i.e., $B_1=0$, $B_2=b_1$, $B_3=b_1+b_2$, \ldots, $B_{n+1}=B$. The encoding function $\mathit{encode}_{\mathcal{Q},\mathcal{R}}:\mathsf{Votes}\rightarrow \{0,\ldots,2^B{-}1\}$ can then be defined as follows:
\begin{align*}
...
To decode an integer representation $x=\mathit{encode}_{\mathcal{Q},\mathcal{R}}(v_i)$ back to $v=(v_{i,1},\ldots,v_{i,s})$, we must decompose the bit string into its components. Mathematically, this decomposition can be written as follows:
\begin{align*}
	\mathit{encode}^{-1}_{\mathcal{Q},\mathcal{R}}(x) = (v_{i,1},\ldots,v_{i,s}),~\text{where}~v_{i,j}=\lfloor x/2^{B_j} \rfloor \bmod{2^{b_j}}.
\end{align*}
...
\begin{landscape}
	...
	\begin{tabularx}{\linewidth}{|c|c|X|c|}
		\hline Notification & Work & Post & Successors\\
		\hline electionDefinition & \multirow{5}{*}{Create the election data when all parts available and publish it.} & \multirow{5}{*}{$\begin{aligned}[t]\mathit{electionData}:\electionTitle, \electionPeriod,\\ C, R, \electionIssues,\\ \mathit{encryptSetting},\mathit{signSetting},\\ \mathit{hashSetting},y, \hat{g}\end{aligned}$ } & \multirow{5}{*}{-} \\ 
		encyptionKey &&& \\
		electionIssues &&& \\
		electoralRoll &&& \\
		mixedKeys &&& \\
