...
\begin{document}
	...
	(work in progress)}
	\maketitle
	% To adjust order of appearance consult:
	% http://mirror.switch.ch/ftp/mirror/tex/macros/latex/contrib/vhistory/doc/vh_sets_en.pdf
	\begin{versionhistory}
		\vhEntry{0.1}{14.07.2012}{Haenni Rolf}{Initial Draft.}
		\vhEntry{0.2}{15.08.2012}{Haenni Rolf}{Complete revision of initial draft.}
		\vhEntry{0.3}{10.09.2012}{Haenni Rolf}{Includes now the case of late registrations.}
		\vhEntry{0.4}{13.03.2013}{Haenni Rolf}{Improved ballot acceptance and closing the urn.}
		\vhEntry{0.5}{04.04.2013}{Haenni Rolf}{Revised section on zero-knowledge proofs.}
		\vhEntry{0.5.1}{30.04.2013}{Haenni Rolf}{Improved notational consistency.}
		\vhEntry{0.5.2}{21.05.2013}{Haenni Rolf}{Two important notational corrections.}
		\vhEntry{0.6}{07.01.2015}{Koenig Reto E.}{Quality: Change of title; re-establishement of the version history.}
		\vhEntry{0.6.1}{15.06.2015}{Koenig Reto E.}{Revised section on randomness.}
		\vhEntry{0.6.2}{16.06.2015}{Hauser Severin}{Revision of UniVote.}
		\vhEntry{0.6.3}{16.06.2015}{Koenig Reto E.}{Revision of UniVote.}
		\vhEntry{0.6.4}{26.06.2015}{Haenni Rolf}{Table in Section 7 adjusted.}
		\vhEntry{0.6.5}{20.07.2015}{Haenni Rolf}{Updated tables in Section 7; updated late registration procedure.}
		\vhEntry{0.6.6}{22.07.2015}{Haenni Rolf}{Final minor protocol adjustments.}
		\vhEntry{0.6.7}{04.08.2015}{Haenni Rolf}{Cleanup in encoding and hashing.}	
		\vhEntry{0.6.8}{18.08.2015}{Haenni Rolf}{Encoding section updated.}	
		\vhEntry{0.6.9}{27.08.2015}{Haenni Rolf}{Password-based key derivation.}	
		\vhEntry{0.6.10}{01.09.2015}{Dubuis Eric}{Reformatted tables for actions and other matters.}
		\vhEntry{0.6.11}{10.01.2016}{Haenni Rolf}{Updated mixing.}
		\vhEntry{0.6.12}{12.01.2016}{Haenni Rolf}{Get statements added for completeness}
	\end{versionhistory}
...
\begin{itemize}
	...
	\item $\mathcal{M}_{\mathit{certificates}}$: Format of a certificate.
\end{itemize}
...
\chapter{UniVote}
%===============================================================================================
\emph{Introductory text to be provided.}
...
\EC performs the following steps: 
\begin{enumerate}
	\item Get $(\mathsf{administrationCertificate}: Z_{\EA})$ from \UBV.
...
\EC performs the following steps:
\begin{enumerate}
	\item Get $(\mathsf{trusteeCertificates}: \mathcal{Z}_{T},\mathcal{Z}_{M})$ from \UBV.
...
\EC performs the following steps:
\begin{enumerate}[resume]
	\item $(\mathsf{trustees}: T, M)$ from \UBV.
	\item For each $\Tallier{j}\in T$:
	\begin{enumerate}
...
\begin{enumerate}
	\item Get $(\mathsf{trusteeCertificates}: \mathcal{Z}_{T},\mathcal{Z}_{M})$ from \UBV.
	\item Select $(\Mixer{1},\vk{1})$ from $Z_{1}\in \mathcal{Z}_{M}$.
	\item Post $(\mathsf{accessRight}:\vk{1},\mathsf{keyMixingResult},1)$ to \UBV.
	\item Select $\mathit{VK}_0=\{\vk{1},\ldots,\vk{n'}\}$ from $\mathcal{Z}_V$.
	\item Let $g_0=g$.
	\item Post $(\mathsf{keyMixingRequest}:\Mixer{1}, \mathit{VK}_0,g_0)$ to \UBV.
\end{enumerate}
The following steps are repeated for every $\Mixer{k}\in M$ (in ascending order for $1\leq k\leq m$):
\begin{enumerate}[resume]
	\item $\Mixer{k}$ performs the following steps:
	...
	\begin{enumerate}
		...
		\item Verify $\pi_{k}$.
		\item Get $(\mathsf{trusteeCertificates}: \mathcal{Z}_{T},\mathcal{Z}_{M})$ from \UBV.
		\item If $k<m$, perform the following steps:
		\begin{enumerate}
			\item Select $(\Mixer{k+1},\vk{k+1})$ from $Z_{k+1}\in \mathcal{Z}_{M}$.
			\item Post $(\mathsf{accessRight}:\vk{k+1},\mathsf{keyMixingResult},1)$ to \UBV.
			\item Post $(\mathsf{keyMixingRequest}:\Mixer{k+1}, \mathit{VK}_{k},g_{k})$ to \UBV.
		\end{enumerate}
		\item If $k=m$, perform the following steps:
		\begin{enumerate}
			\item Let $\hat{\mathit{VK}}=\mathit{VK}_m$ denote the set of mixed public keys and $\hat{g}=g_m$ the \emph{signature generator} for this election.
...
\EC performs the following steps:
\begin{enumerate}
	\item Get $(\mathsf{mixedKeys}:\hat{\mathit{VK}},\hat{g})$ from \UBV.
	...
	\item Get $(\mathsf{cryptoSetting}:\mathit{encryptSetting},\mathit{signSetting},\mathit{hashSetting})$ from \UBV.
	\item Get $(\mathsf{encryptionKey}:y)$ from \UBV.	
...
\subsubsection{Ballot Validation and Vote Mixing}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item Verify $\pi_{i}$.
		\item If $\pi_{i}$ is valid, collect $E_{i}$ into $\mathcal{E}_0$.
	\end{enumerate}
	\item Get $(\mathsf{trusteeCertificates}: \mathcal{Z}_{T},\mathcal{Z}_{M})$ from \UBV.
	\item Select $(\Mixer{1},\vk{1})$ from $Z_{1}\in \mathcal{Z}_{M}$.
	\item Post $(\mathsf{accessRight}:\vk{1},\mathsf{voteMixingResult},1)$ to \UBV.
	\item Post $(\mathsf{voteMixingRequest}:\Mixer{1},\mathcal{E}_{0})$ to \UBV.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			\item Select $(\Mixer{k+1},\vk{k+1})$ from $Z_{k+1}\in \mathcal{Z}_{M}$.
			\item Post $(\mathsf{accessRight}:\vk{k+1},\mathsf{voteMixingResult},1)$ to \UBV.
			\item Post $(\mathsf{voteMixingRequest}:\Mixer{k+1},\mathcal{E}_{k})$ to \UBV.
		\end{enumerate}
		\item If $k=m$, perform the following steps: 
		\begin{enumerate}
			\item Let $\hat{\mathcal{E}}=\mathcal{E}_m$ be the set of mixed encrypted votes. 
...
\begin{enumerate}
	\item Verify $Z'_i$.
	\item Get  $(\mathit{electoralRoll}:V)$ from \UBV.
	\item Select $(\Voter{i},\vkprime{i})$ from $Z'_i$.
...
\begin{enumerate}[]
	\item \EC performs the following steps:
	\begin{enumerate}
		\item Post $(\mathsf{singleKeyMixingRequest}:\Mixer{k}, \vk{i},\vk{i,k-1},g_{k-1},g_k)$ to \UBV.\footnote{We include the original input key $\vk{i}$ for convenience reasons.}
		\item Select $\vk{k}$ from $Z_{k}\in \mathcal{Z}_{M}$.
		\item Post $(\mathsf{accessRight}:\vk{k},\mathsf{singleKeyMixingResult},+1)$ to \UBV.
	\end{enumerate}
	\item $\Mixer{k}$ performs the following steps:
	\begin{enumerate}
		\item Get $(\mathsf{singleKeyMixingRequest}: \Mixer{k}, \vk{i},\vk{i,k-1},g_{k-1},g_k)$ from \UBV.
...
% todo: add automatic page reference
See tables on page 47 and 48.
...
The following parameters are assumed to be known in advance and do not  change over time.
...
This chapter describes the detailed design of UniVote. It starts off by defining the actions which make up the core of the components of UniVote. Later, more design details will be provided.

Actions are triggered either by notification (normal case) or by manual intervention. Upon an notification (\emph{Notification} column in the tables), the following tables define the steps an action undertakes (\emph{Work}), the message it sends to the board (\emph{Post}) and the successor action(s) if any (\emph{Successors}) that may occur as a consequence of terminating the current the action.


\section{EC Setup Phase Actions}

Actions relevant of the setup phase.

	\subsection{Initial}
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\ 
		\hline accessRight & Start the new election & -  & \mbox{DefineEA} \\  
		\hline 
	\end{tabularx} 
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\ 
		\hline EA defined & Search and set cert for EA & administrationCertificate: $Z_{EA}$ & \mbox{GrantElectionDefinition}, \mbox{GrantTrustees}, \mbox{GrantSecurityLevel}, \mbox{GrantElectoralRoll}, \mbox{GrantElectionIssues} \\ 
		\hline 
	\end{tabularx} 
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\ 
		\hline administration\-Certificate & Grant EA access to electionDefinition & accessRight: $vk_{EA}$, \emph{electionDefinition}, 1 & CreateVotingData \\ 
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\ 
		\hline administration\-Certificate & Grant EA access to trustees & accessRight: $vk_{EA}$, \emph{trustees}, 1 & PublishTrusteeCerts \\  
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\ 
		\hline administration\-Certificate & Grant EA access to securityLevel & accessRight: $vk_{EA}$, \emph{securityLevel}, 1 & SetCryptoSetting \\ 
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\  
		\hline securityLevel & Publish the crypto setting for this election & cryptoSetting: \emph{settings} & GrantEncyptionKeyShares \\
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\  
		\hline  trustee\-Certificates & Grant Trustees access to keyshares & accessRight: $vk_t$, \emph{encrytptionKeyShare}, 1 & CombineEncryptionKey\-Shares  \\  
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\  
		\hline  encryption\-KeyShare & Validate key share and if all key shares are published combine them & encryptionKey: $y$ & CreateVotingData \\ 
		\hline 
	\end{tabularx}

\section{EC Preparation Phase Actions}

Actions relevant for the preparation phase of the EC.

	\subsection{GrantElectionIssues}
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\
		\hline administration\-Certificate & Grant EA access for EI & accessRight: $vk_{EA}$, \emph{electionIssues}, 1 & CreateVotingData \\
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
			\hline Notification & Work & Post & Successors\\
			\hline administration\-Certificate & Grant EA access for ER & accessRight: $vk_{EA}$, \emph{electoralRoll}, 1 & KeyMixing \\ 
			\hline 
		\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\
		\hline electoralRoll & Retrieve voter certs and create the first mixing request &
		voterCertificates: $\mathcal{Z}_V$ \newline
		keyMixingRequest: $M_1$, $g_0$, $\mathit{VK}_0$ \newline
		accessRight: $\mathit{vk}_1$, \emph{keyMixingResult}, 1
		& GrantBallot, CreateElectionData \\ 
		\cline{1-3}
		keyMixing\-Request & Create next request or publish mixed keys &
		keyMixingRequest: $M_k,g_k$, $\mathit{VK}_k$ \newline
		accessRight: $\mathit{vk}_k$, \emph{keyMixingResult}, 1 \newline
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\
		\hline mixedKeys & Grant all registered voters access for Ballot & accessRight: $\hat{\mathit{vk}}_i$, \emph{ballot}, 1, \emph{period} & - \\ 
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|p{4.2cm}|X|}
		\hline Notification & Work & Post & Successors\\
		\hline election\-Definition, election\-Details, cryptoSetting, encryptionKey, signature\-Generator & Create the voting data when all parts are available and publish it. & votingData: $\electionTitle$, $\electionPeriod$, $C$, $R$, \emph{electionDetails}, \emph{encryptSetting}, \emph{signSetting}, \emph{hashSetting}, $y$, $\hat{g}$ & - \\ 
		\hline 
	\end{tabularx}

\section{Trustees}

Actions relevant for the trustees. A trustee can be either a tallier of a mixer.

	\subsection{Tallier: CreateSharedKey}
	\begin{tabularx}{\textwidth}{|p{2.1cm}|p{5.5cm}|p{3.0cm}|X|}
		\hline 
		Notification & Work & Post & Successor \\ 
		\hline 
		accessRight: encryptionKeyShare &
		Get crypto setting from UBV \newline
		Store: $x \in_R \mathbb{Z}_Q$ \newline
		$y=G^x$ \newline
		$\pi = \mathit{NIZKP}\{(x):y=G^x\}$ \newline
		finish.
		& KeyShare: $y$  \newline
		Store: $x \in_R \mathbb{Z}_Q$
		& PartialDecryption\\ 
	...
	\begin{tabularx}{\textwidth}{|p{2.1cm}|p{5.5cm}|p{3.0cm}|X|}
		...
		Notification & Work & Post & Successor \\ 
		\hline 
		accessRight: partialDecryption &
		Get mixedVotes $\hat{\mathcal{E}}$ from UBV \newline
		Retrieve: $x$ \newline % correct? (due1)
		$A'=(a_1,\ldots,a_N)\in \mathbb(G)^N_Q $ from $\hat{\mathcal{E}}$ \newline
		$\pi = \mathit{NIZKP}\{(x):y=G^x \wedge (\bigwedge_i a_i=a_i^{-x})\}$ \newline
		finish.
		& partialDecryption: $A$, $\pi$ & -\\
		\hline 
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{|p{2.1cm}|p{5.5cm}|p{3.0cm}|X|}
		...
		\hline 
		keyMixing\-Request &
		Get crpytoSetting from UBV \newline
		Store: $\alpha \in_R \mathbb{Z}_Q$ \newline
		$g=g_{\_}^\alpha$ \newline
		Store: $\psi \in_R \Psi_n$ (permutation vector) \newline
		$\pi=\mathit{NIZKP}\{(\psi,\alpha)\:g=g_{\_}^\alpha \wedge \mathit{VK}=\mathit{shuffle}_\psi (\mathit{VK}_{\_},\alpha)\}$\newline
		finish.
		& keyMixingResult: $g$, $\mathit{VK}$, $\pi$ & Cancelling\-ExistingKey\\ 
		\hline 
	\end{tabularx} 
	...
	\begin{tabularx}{\textwidth}{|p{2.1cm}|p{5.5cm}|p{3.0cm}|X|}
		...
		\hline 
		voteMixing\-Request &
		Get electionSetting: $\mathcal{G}_q,G$ from UBV \newline
		$R=(r_1,\ldots,r_N) \in_R \mathbb{Z}^N_Q $ \newline
		$\tau:[1,N]\rightarrow[1,N] \in_R \Psi_N $ \newline
		$\mathcal{E}=\mathit{shuffle}_\tau(\mathcal{E}_{\_},R)$ \newline
		$\pi=\mathit{NIZKP}\{(\tau,R):\mathcal{E}=\mathit{shuffle}_\tau (\mathcal{E}_{\_},R)\}$ \newline
		finish.
		& voteMixingResult: $\mathcal{E}_{\_}$, $\pi$ & - \\ 
		\hline 
	\end{tabularx} 
	...
	\begin{tabularx}{\textwidth}{|p{2.1cm}|p{5.5cm}|p{3.0cm}|X|}
		...
		ElectionEnd\-Timer &
		run finish.& - & - \\
		&if ElectionEndTimer $\rightarrow$ finish.& & \\ % correct? (due1)
		\hline 
	\end{tabularx} 
	...
	\begin{tabularx}{\textwidth}{|p{2.1cm}|p{5.5cm}|p{3.0cm}|X|}
		...
		Notification & Work &  Post & Successor \\ 
		\hline 
		keyCancelling\-Request, ElectionEndTimer &
		Get $V_i, vk_{i,\_}$ from UVB \newline
		Get $\alpha$ \newline
		$\mathit{vk}_i=\mathit{vk}_{i,\_}^{\alpha}$ \newline
		$\pi=\mathit{NIZKP}\{(\alpha):g=g_{\_}^\alpha \wedge \mathit{vk}_i=\mathit{vk}_{i,\_}^{\alpha}\}$ \newline
		run finish\newline\newline
		if ElectionEndTimer $\rightarrow$ finish.
		& keyCancelling\-Result: $V_i$, $\mathit{vk}_i$, $\pi_i$ & -\\
		\hline 
	\end{tabularx} 
%\end{landscape}
