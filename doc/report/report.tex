...
\begin{itemize}
	\item Post$(i ,m,\alpha):\beta$ \newline This operation allows to publish on UniBoard for application $i$ a message $m \in \mathcal{M}$ and additional arguments $\alpha=(\alpha_1,\dots,\alpha_n)$, where $\alpha_j \in \mathcal{A}_{ij}$. $\mathcal{A}_{ij}$ is the set of possible values for $\alpha_j$ in application $i$. UniBoard might add additional arguments $\beta=(\beta_1,\dots,\beta_k)$, where $\beta_j \in \mathcal{B}_{ij}$, and stores the post $p=(i, m,\alpha,\beta)$ in $\mathcal{P}_i$. UniBoard returns the additional arguments $\beta$ to the author of the message. The arguments in $\alpha$ and $\beta$ depend on the properties supported by UniBoard.
	\item Get$(i, Q):R$ \newline This operation allows to get the published data $\mathcal{P}_i$ from UniBoard for application $i$. The query $Q=(\mathcal{A}'_{i1}, \dots , \mathcal{A}'_{in}, \mathcal{B}'_{i1}, \dots, \mathcal{B}'_{ik})$, where $\mathcal{A}'_{ij} \subseteq  \mathcal{A}_{ij}$ and  $\mathcal{B}'_{ij} \subseteq  \mathcal{B}_{ij}$, allows one to retrieve only a subset $R\subseteq\mathcal{P}_i$, where each $p\in R$ fulfils $\alpha_j \in \mathcal{A}'_{ij}$ and $\beta_j \in \mathcal{B}'_{ij}$. Note that $R$ might also denoted as $\mathcal{P}_i(Q)$. If $Q$ reduces $\mathcal{A}'_{ij}$ to a single value $\alpha_j$, one can also denote this as $\mathcal{P}_i(\alpha_j)$
	
	
%	 The result $R$ for a query $Q=(\mathcal{A}'_{i1}, \dots , \mathcal{A}'_{in}, \mathcal{B}'_{i1}, \dots, \mathcal{B}'_{ik})$, where $\mathcal{A}'_{ij} \subseteq  \mathcal{A}_{ij}$ and  $\mathcal{B}'_{ij} \subseteq  \mathcal{B}_{ij}$, is the set of posts $R\subseteq\mathcal{P}_i$ such that each $p\in R$ fulfils $\alpha_j \in \mathcal{A}'_{ij}$ and $\beta_j \in \mathcal{B}'_{ij}$. $Q$ enables one to retrieve only the information one is interested in by reducing $\mathcal{A}'_1, \dots , \mathcal{A}'_n$ and $\mathcal{B}'_1, \dots , \mathcal{B}'_k$ to the desired values. 
\end{itemize}

%Note that in single-purpose mode its assumed that the application using the board is public known.
%
%In the multipurpose mode UniBoard allows each application $i$ to define its own set of properties needed. Because this also means that $\alpha$ and $\beta$ are different for every application, the application also defines its own sets $\mathcal{A}_{ij}$ and $\mathcal{B}_{ij}$ of possible values for the arguments, where $\alpha_{j} \in \mathcal{A}_{ij}$ and $\beta_{j} \in \mathcal{B}_{ij}$. To define for which application $i$ a post or get operation is intended for, we introduce $i$ into operation signature.
...
%In the multipurpose mode we also place $i$ in the post $p=(i, m, \alpha, \beta)$ and store in $\mathcal{P}_i$, where $\mathcal{P}_i$ is the set of all posts for application $i$.

\section{Post Properties}

\subsection{Sectioned}

Inside a bulletin board with this property, an application can be divided into multiple sections. E.g.: separate data for different customers into individual sections. To realize this property, one must provide the section $s\in\mathcal{S}$ when posting a message, where $\mathcal{S}$ is the set of all existing sections for an application. All posts for section $s$ are stored in $\mathcal{P}_s$ for single purpose mode or $\mathcal{P}_{is}$ for multipurpose mode.

\subsection{Grouped}

In a grouped bulletin board, messages are organized into groups $\mathcal{G}$ defined by the application. So a group $g$ must be provided on posting a message. So in a multipurpose board $\mathcal{G}_{i}$ represents the set of all existing groups for application $i$. 

\subsection{Typed}

This property depends on the grouped property. It defines for each group $g$ the set of correct messages $\mathcal{M}_g$, enabling the board to validate that each incoming message $m$ for group $g$ is in $\mathcal{M}_g$. Thus the board only accepts valid messages.

\subsection{Access-controlled}
The goal of the access-controlled property is to allow only authorized authors to publish messages on the board. So first the author of the message must be identified. Therefore one has now to provide with each message a $\sigma$, which is a signature of the message, and $k$, which represents his public key.

To enable the board to check, if the author is allowed to post the message, we introduce $\mathcal{K}$, which is the set of all public keys $k$ allowed to publish messages on the board. This set can be defined either static or dynamic.

\begin{itemize}
	\item static - the set $\mathcal{K}$ is publicly known and can not be changed.
	\item dynamic - the set $\mathcal{K}$ is defined as the function $f(p, \mathcal{P}, t)$, where $t$ is the current time of the board.
\end{itemize}
 
 Each application and/or section defines its own $\mathcal{K}_i$, $\mathcal{K}_s$ or $\mathcal{K}_{is}$.
 \subsection{Ordered/Sequential}
 
 The ordered property ensures that all published posts $\mathcal{P}$ of an application have a total order by arrival. This is achieved by adding a sequence number $n\in\mathbb{N}$ to each post.
 ...
 The append-only property ensures that no published message can be removed from the board. So, $\mathcal{P}_t \subseteq \mathcal{P}_{t+1}$ where $\mathcal{P}_{t} $ represent the content on the board at time $t$. The solution used in UniBoard requires that the board has the ordered property. In UniBoard it creates a hash-chain $H$ for $\mathcal{P}$. For each $p_i$ it creates an hash $h_i\in{H}$ which includes the preceding hash $h_{i-1}$ and $p_i$. This hash $h_i$ is added to the post as $\beta_{i2}$ and consequently returned to the author.
 
 \begin{itemize}
 	\item Post$(i, m, (\alpha_{i1}, \alpha_{i2}, \alpha_{i3}, \alpha_{i4})):(\beta_{i1}, \beta_{i2})$
 	\item Get$(i, (\mathcal{A}'_{i1}, \mathcal{A}'_{i2}, \mathcal{A}'_{i3}, \mathcal{A}'_{i4})):R$ 
 \end{itemize}
...
Upon posting a message $m$, UniBoard creates the signature $S_m(m,i,A,B)$ including the message, the application identifier $i$ and the $A$ and $B$ arguments. This signature is returned in $\beta_{i3}$. So, the Post operation becomes:

 \begin{itemize}
 	\item Post$(i, m, (\alpha_{i1}, \alpha_{i2}, \alpha_{i3}, \alpha_{i4})):(\beta_{i1}, \beta_{i2}, \beta_{i3})$
 	\item Get$(i, (\mathcal{A}'_{i1}, \mathcal{A}'_{i2}, \mathcal{A}'_{i3}, \mathcal{A}'_{i4})):R$ 
 \end{itemize}
...
% A_Univovte = (S, G, SIGMA_A, K) etc

% Step by step explanation of the message processing of the board.
% 0. Check applications
% 1. Check section
% 2. Check if group exists
% 3. Check if message is of correct type
% 4. Check if signature is correct
% 5. Check if authorisations is ok
