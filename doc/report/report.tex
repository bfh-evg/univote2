...
This section first presents some of the most important properties a bulletin board can have and then describes which ones have been selected for UniBoard configuration in UniVote.
...
Usually a bulletin board supports only one application(e.g. traditional electronic bulletin board).
Multipurpose allows multiple applications to use the same bulletin board. This is achieved by defining an $\mathcal{A}_i$ as the set of all valid application ids.
...
Inside a bulletin board with this property, an application can be partitioned into multiple sections. E.g.: separate data for different customers into individual sections. To realize this property, a set $\mathcal{A}_i$ is defined as the set of all section identifiers for the given application.

\subsection{Structured}

In a structured bulletin board, an application defines a set of message types $\mathcal{A}_i$. A
...
When querying the board, it creates a signature $S_q(Q,R,t_\UB)$ over the query $Q$ the result $R$ and the timestamp $t_\UB$ of the query process. For this property, the Get operation must be adapted as follows:

\begin{itemize}
	\item Get$(Q): (R, S_q, t_\UB)$
\end{itemize}

\subsection{Certified Posting}

Upon posting a message $m$ the \UB creates the signature $S_m(m,A,B,t_\UB)$ including the message, the $A$ and $B$ arguments and the current timestamp $t_\UB$. Timestamp and signature are returned. So, the Post operation must be adapted and becomes:

\begin{itemize}
	\item Post$(m,A): (B, S_m,t_\UB)$
\end{itemize}
...
\section{Properties of UniBoard}
