...
\begin{itemize}
	\item Post$(m,A):B$ \newline This operation allows to publish on the \UB a message $m \in \mathcal{M}$ and additional arguments $A=(\alpha_1,\dots,\alpha_n)$, where $\alpha_i \in \mathcal{A}_i$. The \UB might add additional arguments $B=(\beta_1,\dots,\beta_k)$, where $\beta_i \in \mathcal{B}_i$, and stores the tuple $(m,A,B)$. The \UB returns the additional arguments $B$. The arguments in $A$ and $B$ depend on the properties supported by the \UB.
	\item Get$(Q):R$ \newline This operation allows to get the published data from the \UB as result $R$. The result $R$ is a set of tuples $(m,A,B)$. The query $Q=(\mathcal{A}'_1, \dots , \mathcal{A}'_n, \mathcal{B}'_1, \dots, \mathcal{B}'_k)$, where $\mathcal{A}'_i \subseteq  \mathcal{A}_i$ and  $\mathcal{B}'_i \subseteq  \mathcal{B}_i$, enables one to retrieve only the information one is interested in by reducing $\mathcal{A}'_1, \dots , \mathcal{A}'_n$ and $\mathcal{B}'_1, \dots , \mathcal{B}'_k$ to the desired values. If $\mathcal{A}'_i$ or $\mathcal{B}'_i$ is missing in $Q$ it is assumed to be equal to $\mathcal{A}_i$ or $\mathcal{B}_i$ respectively.
\end{itemize}
