...
\chapter{UniBoard}
%===============================================================================================
A bulletin board can have many different properties. To give one flexibility in their bulletin board implementation UniBoard specifies a common interface and a component model. This allows one to combine different properties as the need arrives or to write their own implementation of a certain property.

This section first presents the basic general operations supported by UniBoard and then describes which properties UniBoard configuration must have in UniVote.

%\section{Properties}
\section{UniBoard basic operations}

UniBoard supports two basic operations.
\begin{itemize}
	\item Post$(m,A):B$ \newline This operation allows to publish on UniBoard a message $m \in \mathcal{M}$ and additional arguments $A=(\alpha_1,\dots,\alpha_n)$, where $\alpha_i \in \mathcal{A}_i$. UniBoard might add additional arguments $B=(\beta_1,\dots,\beta_k)$, where $\beta_j \in \mathcal{B}_j$, and stores the post $p=(m,A,B)$ in $\mathcal{P}_{UB}$. UniBoard returns the additional arguments $B$. The arguments in $A$ and $B$ depend on the properties supported by UniBoard.
...
These properties will be described below showing which effect they have on the basic operations described earlier.

\subsection{Multipurpose}

Usually a bulletin board supports only one application(e.g. traditional electronic bulletin board). Multipurpose allows multiple applications to use the same bulletin board. The data stored on the bulletin board can look very different for each application in accordance with the board properties needed by the applications. So, the basic operations must be extended with an application identifier $i$ in order to be able to define independent $\mathcal{A}_i$ and $\mathcal{B}_i$ for each application. Thus, the operations look like:
...
TODO add a method returning all applications?

%This is achieved by defining an $\mathcal{A}_i$ as the set of all valid application ids.

%With a multipurpose bulletin board, the resting $\mathcal{A}_i$ and $\mathcal{B}_i$ can be defined differently for each application. Therefore, we define Post$_m$ and Get$_m$ as the operations for application $m$.

\subsection{Sectioned}

Inside a bulletin board with this property, an application can be divided into multiple sections. E.g.: separate data for different customers into individual sections. To realize this property, $\alpha_1$ is used as section identifier. So, $\mathcal{A}_1$ represents the set of all section identifiers for the given application.
...
In a grouped bulletin board, messages are grouped inside a section into groups defined by the application. A group identifier must be provided on posting. Therefore, $\alpha_2$ is used as group identifier. So, $\mathcal{A}_2$ represents the set of all possible group identifiers. 
...
Upon posting a message $m$ UniBoard creates the signature $S_m(m,i,A,B,t_{\mathcal{UB}})$ including the message, the application identifier $i$, the $A$ and $B$ arguments and the current timestamp $t_\mathcal{UB}$. Timestamp and signature are returned. So, the Post operation must be adapted and becomes:
