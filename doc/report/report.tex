...
\end{itemize}
 ...
 The ordered property ensures that all published posts $\mathcal{P}_i(\langle s\rangle)$ for a section of an application have a total order. This is achieved by adding a sequence number $n\in\mathbb{N}$ to each post, where $n=|\mathcal{P}_i(\langle s\rangle)|$.
...
Upon posting a message $m$, the board creates the signature $\sigma_p= Sign_{BB}(i, m, \alpha,\beta)$ including the application identifier $i$, the message, and the $\alpha$ and $\beta$ arguments. This signature is returned in $\beta$.

\section{Further Properties}

\subsection{Certified Reading}

This property forces the board to commit to every response $R$ it returns. When querying the board, it creates a signature $\sigma_q(Q,R,t)$ over the query $Q$, the result $R$ and the timestamp $t$ when the query was processed. For this property, the Get operation must be changed as follows:

\begin{itemize}
	\item Get$(i, Q): (R, t, \sigma_q)$
\end{itemize}
...
%This property requires the typed property, as it allows to query not only the arguments $\alpha$ and $\beta$, but also the contents of the message $m$. So $Q$ needs to be expanded with $\gamma$ which must be found in the message $m$ itself. The Get operation must be modified as follow:
%
%\begin{itemize}
...
For UniVote and UniCert we want a multipurpose board the supports all properties described in the previous sections. This results in four operations. 
%TODO More text

Post$(i, m, (s, g, \sigma_a, k):(n, t , h, \sigma_p)$
\begin{enumerate}
	\item Check application identifier $i$.
...
UniCert is a software component playing the role of certification authority. It can be used to issue digital certificates that can then be used in an application. UniCert issues X.509 certificate in PEM format and publishes them on UniBoard in a UniCert specific area. UniCert provides an interface where the certificate requester can authenticate themselves and request a certificate corresponding to their needs.

\paragraph*{Authentication} Authentication in UniCert can be done through various identity providers. The requester has to authenticate themselves to one of these providers which returns the resulting authentication token to UniCert. Based on the identity informations contained in the token, a certificate can be issued. 

\paragraph*{Certificate usage} Each issued certificate can be use for a specific purpose also called usage (e.g. voter certificate, tallier certificate, ...) which is mentioned in the certificate. The usage is linked to a given cryptographic setup (e.g specific $p,q$ and $g$ values for a DSA certificate). So, when requesting a certificate, the requester must indicate a usage and compute a private key corresponding to the setup linked with the selected usage. Then, the requester can submit a certificate signing request (CSR) indicating the public key corresponding to the private key computed, the usage and an authentication token.

Before signing the CSR submitted by the requester, UniCert has to verify if the content of the CSR correspond to the setup corresponding to the selected usage. Following properties must be checked:
\begin{itemize}
	\item check if the same cryptographic setup as the one specified by the selected usage is used
	\item check if the requester knows the private key corresponding to the public of the CSR submitted
	\item check if the distinguished name of the requester is in the format required by the usage
\end{itemize}

This last point is needed for data protection reasons. Since the issued certificate are published on UniBoard, one could require the certificate to be anonymized. This can be defined during the creation of a new usage. When requested, a hash of the mail address can be used in the certificate instead of the mail address itself.
