...
\todo{Does the requester submit a CSR, or does he only send the public key and the usage?}
...
%\todo{Is it necessary to describe the interface more in details?}

\todo{From where are the "usage" possibilities read? From each section for UniVote in UniBoard? => read over multiple sections... Theoretically not, because the setup is not related to an election but to a group of elections.}

\todo{Who defines the usages? The Election Coordinator?}

\todo{Must the authentication token be explicitly given to UniCert or is it done implicitly}

\todo{How is the link between crypto setup and usage done? And what about other properties?}

\todo{Format of the public key to submit? Format of the certificate?}

\todo{Describe which field of X.509 certificate contains what?}

\todo{Which hash method is used for DN?}
