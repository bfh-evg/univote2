...
\EA performs the following steps:
\begin{enumerate}[resume]
	\item Define the set of choices $C=\{c_1,\ldots,c_s\}$ and a rule set $R$ describing the set $\mathsf{Votes}=\mathit{electionOptions}(C,R)$ of valid \emph{election options}, where $\mathit{electionOptions}$ is a publicly known function (see \secref{choices} for details).
...
\begin{enumerate}
	...
	\item Compute $w_i=\mathit{encode}_{C,R}(v_i) \in \mathbb{Z}_Q$.
...
\begin{enumerate}[resume]
	...
	\item For all $1\leq i\leq N$, compute $v_i = \mathit{decode}_{C,R}(w_i)$.
	\item Let $\mathcal{V}=\{v_1,\ldots,v_{N}\}\cap \mathsf{Votes}=\{v'_1,\ldots,v'_{N'}\}$ be the list of $N'\leq N$ valid plaintext votes $v'_i=(v'_{i,1},\ldots,v'_{i,s})$, where $v'_{i,j}$ denotes the number of votes for $c_j$.
...
\begin{enumerate}
	...
	\item Post $(\mathsf{accessRight}:\vkhatprime{i}, \mathsf{ballot}, 1,\period)$ to \UBV.
\end{enumerate}
...
\begin{itemize}
	...
	\begin{align*}
		C=\{yes_1,no_1,\ldots,yes_s,no_s\},~ 
		R=\left\{\begin{aligned}&\Sigma:\{yes_1,no_1\}\rightarrow[1,1] \\
		&~~~\vdots~~~~~~~~~~~\vdots~~~~~~~~~~~\vdots\\
...
For $C=\{c_1,\ldots,c_s\}$, let $v_{i,j}\leq \rho_j$ the number of votes attributed to $c_j$ by a given voter $\Voter{i}$. With $\rho_j$ we denote the maximum number of votes that the election rules in $R$ allow for $c_j$. The tuple $v_i=(v_{i,1},\ldots,v_{i,s})\in\mathsf{Votes}$ represents a valid vote of \Voter{i}, where $\mathsf{Votes}=\mathit{electionOptions}(C,R)$ denotes the set of all valid votes for given sets $C$ and $R$.

To encode $v_i\in \mathsf{Votes}$ as an integer, consider a bit string of length $B=\sum_{j=1}^n b_j$, where $b_j=\lfloor\log_2\rho_j\rfloor+1$ denotes the number of bits reserved for each value $v_{i,j}$. Furthermore, let $B_j=\sum_{k=1}^{j-1}b_k$ be the number of bits in the bit string \emph{prior} to $v_{i,j}$, i.e., $B_1=0$, $B_2=b_1$, $B_3=b_1+b_2$, \ldots, $B_{n+1}=B$. The encoding function $\mathit{encode}_{C,R}:\mathsf{Votes}\rightarrow \{0,\ldots,2^B{-}1\}$ can then be defined as follows:
\begin{align*}
...
%	
%	\begin{align*}
%		\Sigma_\mathsf{ENC} &= \{\mathsf{RC0}, \mathsf{RC1}, \mathsf{RC2}, \mathsf{RC3}, \mathsf{EC3}, \mathsf{EC4}, \mathsf{EC5}\} \\
%		\Sigma_\mathsf{SIG} &=  \{\mathsf{RC0s}, \mathsf{RC1s}, \mathsf{RC2s}, \mathsf{RC3s}, \mathsf{EC3}, \mathsf{EC4}, \mathsf{EC5}\} \\
...
%	\end{align*}
	...
	\begin{align*}
		\Sigma_\mathsf{ENC} &= \{\mathsf{RC0e}, \mathsf{RC1e}, \mathsf{RC2e}, \mathsf{RC3e}\} \\
		\Sigma_\mathsf{SIG} &= \{\mathsf{RC0s}, \mathsf{RC1s}, \mathsf{RC2s}, \mathsf{RC3s}\} \\
