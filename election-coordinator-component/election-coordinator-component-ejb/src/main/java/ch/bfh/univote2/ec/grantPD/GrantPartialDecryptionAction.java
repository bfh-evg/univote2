 ...
 */
@Stateless
public class GrantPartialDecryptionAction implements NotifiableAction {

	private static final String ACTION_NAME = GrantPartialDecryptionAction.class.getSimpleName();
	private static final Logger logger = Logger.getLogger(GrantPartialDecryptionAction.class.getName());
	...
	public ActionContext prepareContext(String tenant, String section) {
		...
		GrantPartialDecryptionActionContext actionContext = new GrantPartialDecryptionActionContext(ack);
	...
	public void run(ActionContext actionContext) {
		...
		if (actionContext instanceof GrantPartialDecryptionActionContext) {
			GrantPartialDecryptionActionContext gedac = (GrantPartialDecryptionActionContext) actionContext;
	...
	public void notifyAction(ActionContext actionContext, Object notification) {
		if (actionContext instanceof GrantPartialDecryptionActionContext) {
			GrantPartialDecryptionActionContext geksac = (GrantPartialDecryptionActionContext) actionContext;
	...
	protected void parseTrusteeCerts(TrusteeCertificates trusteeCertificates,
			GrantPartialDecryptionActionContext actionContext)
	...
	protected boolean grantAccessRight(ActionContext actionContext, PublicKey publickey) {
		try {
			byte[] message = MessageFactory.createAccessRight(GroupEnum.PARTIAL_DECRYPTION, publickey, 1);
