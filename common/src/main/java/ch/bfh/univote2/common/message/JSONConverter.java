...
/**
 * Utility class that allows to convert JSON byte[] messages to objects of domain classes as well as to convert these
 * instances to JSON message strings.
 * <p>
 * An instance of this class supports support the type query predicate {@link #isOfType(java.lang.Class, byte[])}. If
 * called with a type and a message, <code>true</code> is returned if the message matches the specified type,
 * <code>false</code> otherwise. On success, the domain object for the given message is cachen and can be retrieved by
 * calling {@link #getUnmarshalledMessage()}.
 *
...
public class JSONConverter {
	...
	/**
	 * Checks if the getters of the domain object are not null. If at least one required getter returns null then assume
	 * that the message does not match the given type and, therefore, this method returns false.
	 *
